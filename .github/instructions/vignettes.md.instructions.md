...
4. **HierarchicalBMA.Rmd** - Multilevel models via `study_ids`
5. **MedicineBMA.Rmd** - Informed priors for medical meta-analysis (continuous outcomes)
6. **MedicineBiBMA.Rmd** - Informed priors for binary outcomes (log OR, RR, RD, HR)
7. **CustomEnsembles.Rmd** - Advanced ensemble customization
8. **FastRoBMA.Rmd** - Spike-and-slab algorithm (`algorithm = "ss"`)
9. **ZCurveDiagnostics.Rmd** - Meta-analytic z-curve publication bias diagnostics
