...
{
  if (len.size() != 19) return false;
  const unsigned int maximum = std::numeric_limits<int>::max();
  for (unsigned int length : len) if (length > maximum) return false;
  for (unsigned int i = 0; i < 6; ++i) if (len[i] != 1) return false;
  for (unsigned int i = 9; i < 13; ++i) if (len[i] != 1) return false;
  if (len[18] != 1) return false;
  return len[6] == len[7] &&
    len[7] == len[8] &&
    len[13] == 2 &&
    len[14] == 2 &&
                                         ...
                                         std::vector<unsigned int> const &len) const
{
  return *par[1] > 0 && *par[3] > 0 && *par[4] > 0 && *par[5] > 0 &&
    (*par[10] == 1 || *par[10] == -1) &&
    (*par[11] >= 0 && *par[11] < 1) &&
    selnorm_jags_integer_in_range(*par[12], 0, 2) &&
    selnorm_jags_integer_in_range(*par[18], SELKERNEL_NORMAL, SELKERNEL_STEP_PHACK_POWER) &&
    // Zero remains valid when no step weight is active; no p-hacking mode is
    // introduced here. The existing kernel retains its supported-mode routing.
    selnorm_jags_integer_in_range(*par[9],
      (*par[18] == SELKERNEL_STEP || *par[18] == SELKERNEL_STEP_PHACK_POWER) ? 1 :
        std::numeric_limits<int>::min(),
      (*par[18] == SELKERNEL_STEP || *par[18] == SELKERNEL_STEP_PHACK_POWER) ?
        static_cast<int>(len[6]) : std::numeric_limits<int>::max());
}
