...
for more information). The package automatically standardizes the
continuous moderators. This achieves scale-invariance of the specified
prior distributions and ensures that the prior distribution for the
intercept correspond to the grand mean effect. This setting can be
overridden by specifying the `standardize_predictors = FALSE` argument.
...
[`?RoBMA.reg`](https://fbartos.github.io/RoBMA/reference/RoBMA.reg.md)
for more information). The `"meandif"` contrasts achieve label
...
allows specifying `"treatment"` contrasts, which result in a prior
distribution on the difference between the default level and the
...
function, this is achieved by the `d`, `r`, `logOR`, `OR`, `z`, `se`,
`v`, `n`, `lCI`, and `uCI` arguments. The input is passed to the
...
[`RoBMA.reg()`](https://fbartos.github.io/RoBMA/reference/RoBMA.reg.md)
(and
[`NoBMA.reg()`](https://fbartos.github.io/RoBMA/reference/NoBMA.reg.md))
function requires the dataset to be passed as a data.frame (without
missing values) with column names identifying the - moderators passed
using the formula interface (i.e., `~ measure + age` in our example) -
and the effect sizes and standard errors (i.e., `r` and `se` in our
example).
...
[`RoBMA.reg()`](https://fbartos.github.io/RoBMA/reference/RoBMA.reg.md)
function that automatically removes models adjusting for publication
bias). We specify the model formula with the `~` operator similarly to
the `rma()` function and pass the dataset as a data.frame with named
columns as outlined in the section above (the names need to identify the
moderators and effect size measures). We also set the `parallel = TRUE`
argument to speed up the computation by running the chains in parallel
...
Note that the
[`NoBMA.reg()`](https://fbartos.github.io/RoBMA/reference/NoBMA.reg.md)
function specifies the combination of all models assuming presence
vs. absence of the effect, heterogeneity, moderation by `measure`, and
moderation by `age`, which corresponds to $`2*2*2*2=16`$ models.
Including each additional moderator doubles the number of models,
leading to an exponential increase in model count and significantly
...
#>     study_names = study_names, cluster = cluster, transformation = transformation, 
...
$`1/0.245 = 4.08`$). These findings extend the frequentist
meta-regression by disentangling the absence of evidence from the
evidence of absence.
...
zero, $`\text{BF}_{10} = \infty`$. The test is performed using the
change from prior to posterior distribution at 0 (i.e., the
Savage-Dickey density ratio) assuming the presence of the overall effect
or the presence of difference according to the tested factor. Because
the tests use prior and posterior samples, calculating the Bayes factor
can be problematic when the posterior distribution is far from the
tested value. In such cases, warning messages are printed and
...
### Robust Meta-Regression with Publication-Bias Adjustment
...
[`RoBMA.reg()`](https://fbartos.github.io/RoBMA/reference/RoBMA.reg.md)
function with the same arguments as in the previous section. The
estimation time further increases as the ensemble now contains 144
models.
...
$`\text{BF}_{\text{pb}} = 28.0`$. Furthermore, accounting for
publication bias turns the previously found evidence for the overall
effect into a weak evidence against the effect $`\text{BF}_{10} = 0.51`$
and notably reduces the mean effect estimate $`\rho = 0.032`$, 95% CI
\[0.000, 0.164\].
