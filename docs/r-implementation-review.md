...
Review: The JAGS backend is essentiial for any model fitting, the user should be notified early on propetly if the module failed to install/load
...
Review: examine BayesTools, this seems like it should be based on all the computations and transformations done in BayesTools interface. Do not implement anything yet but report back with instruction that I can give to my BayesTools agent to implement this if that is the best approach

### Outcome helpers live in the wrong module

`.outcome_data_yi()`, `.outcome_data_sei()`, `.outcome_data_vi()`, and related helpers live in `R/residuals.R`, but are used by diagnostics, plots, pdf/cdf, zplot, and VIF code.
...
Review: we can easily add functionality to BayesTools, suggest what is needed where, I will forward it to the BayesTools agent, examine what is needed from bridgesampling exactly, and suggest specific local solutions
...
Review: document flattening and drop a meesage, there is no other coherent solution
...
Review: these are different, bias_adjusted returns the object adjusted for bias, sampling bias alters the visualized sampling distrubtion
...
`as_zplot()` rejects GLMM objects before lower-level zplot code reads `outcome$yi` and `outcome$sei`.
...
Review: add a note that we want to implement show_data for the weightfunction in BayesTools, remove the is_multilevel  etc...
...
Review: exoise `sei` which accepts a single real value for the plotting, change the default to the median(se) and document
...
Review: examine whether there is a close form solution that could be used instead?
