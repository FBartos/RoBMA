...
The bias side of the ensemble puts the largest posterior weight on PET (posterior probability $0.68$, individual inclusion BF $\approx 15$), followed by PEESE ($0.21$); the remaining bias components together account for about $0.10$.
...
These numerical weights remain approximate: the example fit reports R-hat values up to 1.08 for bias-state indicators and PET/PEESE coefficients, and the heterogeneity indicator is constant, so its convergence diagnostics cannot be assessed.
