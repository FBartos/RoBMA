...
Bayesian model-averaged meta-analysis allows researchers to seamlessly incorporate available prior information into the analysis [@gronau2017bayesian; @gronau2020primer; @bartos2021bayesian]. 
...
The specified prior can be printed by setting the `only_priors = TRUE` argument.
...
We ignore this section and move to the last part.
...
By default, the function plots the model-averaged estimates across all models; the arrows represent the probability of a spike, and the lines represent the posterior density under models assuming non-zero effect. 
The secondary y-axis (right) shows the probability of the spike (at value 0) decreasing from 0.50, to 0.005 (also obtainable from the model summary).
