...
date: "2022 (updated on 27th of April 2026)"
output:
...
fit_RoBMA <- RoBMA(
  yi = yi, vi = vi, measure = "ZCOR", data = dfz,
  prior_effect        = prior("normal", parameters = list(mean = 0, sd = 1)),
  prior_heterogeneity = prior("invgamma", parameters = list(shape = 1, scale = 0.15)),
  model_type = "PSMA", seed = 1, parallel = TRUE, save = "min")
...
fit_RoBMA2 <- RoBMA(
  ...
  model_type = "PSMA", seed = 2, parallel = TRUE, save = "min")
...
**This vignette was updated on 27th of April to be compatible with the updated version of the package. Due to changes in algorithms and MCMC settings, the output no longer exactly matches the published manuscript.**
...
To apply the transformation, we use the `escalc()` function from the `metafor` package. We pass the correlation coefficients into the `r` argument, the sample sizes to the `n` argument, and set the `transformation` argument to `"fishers_z"` (the `study_names` argument is optional). The function `combine_data()` then saves the transformed effect size estimates into a data frame called `dfz`, where the `y` column corresponds to Fisher's *z* transformation of the correlation coefficient and `se` column corresponds to the standard error of Fisher's *z*.
