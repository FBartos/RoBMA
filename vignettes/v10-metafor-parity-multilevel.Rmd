...
The `cluster` argument provides a specialized way to fit three-level models through the univariate `brma()` interface, without requiring the more general `brma.mv()` specification.
It supports one cluster grouping above the estimate level and parameterizes the model by total heterogeneity $\tau^2$ and cluster-level allocation $\rho$.
Known sampling covariance matrices and richer random-formula structures use the separate `brma.mv(..., V = ..., random = ...)` interface; `brma.mv()` intentionally does not accept `cluster`.
See the [*Multivariate and Multilevel Meta-Analysis*](v14-metafor-parity-multivariate.html) vignette for those models.
...
`brma(..., cluster = ...)` is the specialized interface for this model.
The same covariance can be represented with `brma.mv()`, but the narrower `cluster` parameterization provides dedicated output for $I^2$ and its within- and between-cluster components.
...
For known sampling covariance matrices and richer random-effects structures, see the [*Multivariate and Multilevel Meta-Analysis*](v14-metafor-parity-multivariate.html) vignette.
