...
Selection models are implemented in `weightr` package (`weightfunct()` function; @weightr) and newly also in the `metafor` package (`selmodel()` function; @metafor). 
...
First, we check the test for heterogeneity that clearly rejects the null hypothesis `Q(df = 17) = 75.4999, $p$ = 5.188348e-09` (if we did not find evidence for heterogeneity, we could have proceeded by fitting the fixed effects version of the model by specifying the `fe = TRUE` argument). 
...
These prior distributuions reproduce the version 3.6.1 specification after the old approximate Cohen's *d* to Fisher's *z* prior rescaling. 
...
The 4.0 version of the package uses product-space parameterization with notably speeds up the sampling an takes usually under a minut for moderately large datasets. 
...
The second column summarizes the prior model probabilities of models assuming either presence of the individual components. 
...
Evidence for the presence of an effect is undecided; the models assuming the presence of an effect are only ~1.3 times more likely given the data than the models assuming the absence of an effect. 
...
If we were interested in the estimates model-averaging only across the models assuming the presence of an effect, heterogeneity, and publication bias, we could add the `conditional = TRUE` argument to the summary function.
...
The `RoBMA` package allows us to fit ensembles of highly customized meta-analytic models. 
...
We explicitly specify the prior distribution for models assuming presence of the effect with the `prior_effect` argument, which assigns Normal(0.30, 0.10) distribution bounded to positive values to the $\mu$ parameter on the Fisher's *z* scale. 
