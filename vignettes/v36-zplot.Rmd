...
3. **ChatGPT effects on learning performance** [@wang2025effect]: an example with extreme evidence for publication bias but a small conventional `Missing N` diagnostic.
...
We prepare each diagnostic once and reuse it to avoid repeated calculations for this large multilevel example:
...
This is achieved by calling the `zplot()` function with `plot_extrapolation = TRUE`, which accepts either a fitted model or a prepared `zplot_brma` object.
...
The large `Missing N` diagnostic describes substantial extrapolation under the convention above; it does not identify how many studies are absent from the literature.
...
The large `Missing N` diagnostic indicates substantial conventional extrapolation, subject to the reporting assumptions above; it is not an identified count of absent studies.
...
The model comparison below provides extreme evidence for publication bias; the extrapolation panel shows its effect on the implied distribution.
...
The random-effects model yields a substantial positive effect size estimate, while RoBMA produces a smaller estimate with a wider credible interval that includes zero.
Both models retain substantial between-study heterogeneity.
The Bayes factor for the effect is 1.06, leaving the evidence undecided, whereas the Bayes factor of roughly 3,000 indicates extreme evidence for publication bias.
...
The summary shows a moderate-to-high EDR and a small conventional `Missing N` diagnostic.
The small `Missing N` diagnostic describes the inverse-weight extrapolation convention and does not imply weak evidence for publication bias.
...
Both models yield similar effect size estimates.
...
**Zplot summary metrics.**
The publication-bias assessment statistics show a moderate EDR matching the ODR and a near-zero conventional `Missing N` diagnostic; the latter does not establish that no studies are missing.
