...
vignette_cache_save(cached_fits["fit_RE_Weingarten2018"])
...
vignette_cache_save(cached_fits["fit_RE_Hoppen2025"])
...
vignette_cache_save(cached_fits["fit_RoBMA_Hoppen2025"])
...
vignette_cache_save(cached_fits["fit_RE_Wang2025"])
...
vignette_cache_save(cached_fits["fit_RoBMA_Wang2025"])
...
vignette_cache_save(cached_fits["fit_RE_ManyLabs16"])
...
vignette_cache_save(cached_fits["fit_RoBMA_ManyLabs16"])
...
vignette_cache_save(cached_fits["fit_RoBMA_Weingarten2018"])
```
...
The method also provides a bias-adjusted reference for the expected discovery rate and, for the univariate product models used here, the conventional `Missing N` diagnostic.

`Missing N` retains reference-bin-normalized inverse-weight extrapolation for independent models and the released conditional-cluster product model with estimate random effects integrated.
Resolving the same publication groups automatically or explicitly does not change this diagnostic or the reference distribution.
Its literal count interpretation additionally treats the reference bin as fully observed and requires a reporting/stopping design linking attempted to observed estimates.
Those assumptions are not part of the fitted relative-weight likelihood: the model conditions on the reported vector and does not identify an absolute publication probability or a finite pool of missing studies.
Valid relative weights above one can even produce a negative diagnostic, which cannot be read as a literal count.
Multivariate selection models, models conditioning on estimate random effects, models integrating applicable other random effects or sampling context, and best-rule selection models instead use a normalized pre-selection Gaussian reference and do not report `Missing N`.
...
3. **ChatGPT effects on learning performance** [@wang2025effect]: an example with substantial publication-bias adjustment but a small conventional `Missing N` diagnostic.
...
The example fit reports all three component Bayes factors as $>49{,}999$ because their excluded states were not visited; these are reporting bounds, and convergence diagnostics for the constant indicators are unavailable.
Some bias-state indicators also have R-hat values up to 1.15, so the model probabilities should not be treated as numerically settled.
...
We prepare each diagnostic once and reuse it, avoiding repeated discovery-rate calculations for this large multilevel example.

```{r, fig.cap="Ease-of-Retrieval Effect: Model Fit Assessment"}
zplot_RE_Weingarten2018    <- as_zplot(fit_RE_Weingarten2018)
zplot_RoBMA_Weingarten2018 <- as_zplot(fit_RoBMA_Weingarten2018)

hist(zplot_RoBMA_Weingarten2018, from = -3, to = 6, by = 0.25)

lines(zplot_RE_Weingarten2018,    from = -3, to = 6, col = "black", lty = 2, lwd = 2)
lines(zplot_RoBMA_Weingarten2018, from = -3, to = 6, col = "blue",  lty = 2, lwd = 2)
...
This is achieved by calling the `zplot()` function with `plot_extrapolation = TRUE`; it accepts either a fitted model or a prepared `zplot_brma` object.
The extrapolated curve is not drawn by default because it rescales the display by an inferred selection convention rather than describing the observed literature.

```{r, fig.cap="Ease-of-Retrieval Effect: Extrapolation Analysis"}
zplot(zplot_RoBMA_Weingarten2018, from = -3, to = 6, by.hist = 0.25, plot_extrapolation = TRUE)
```

Under the inverse-weight convention, the blue line extrapolates the fitted display after removing publication-bias adjustments; it need not integrate to one.
...
```{r}
summary(zplot_RoBMA_Weingarten2018)
```

The summary provides several key results.
The observed discovery rate (ODR = 0.45) substantially exceeds the posterior mean expected discovery rate (EDR = 0.21, 95% CrI [0.16, 0.27]), indicating that more significant results appear in the published literature than the adjusted reference distribution predicts.
The `Missing N` diagnostic has posterior mean 511 and 95% CrI [247, 957], describing substantial extrapolation under the convention above; it does not identify how many studies are absent from the literature.
...
The publication-bias Bayes factor is approximately 450, with estimated relative Monte Carlo error of 21%.
The example fit retains bias-state R-hat values up to 1.17 and an unvisited excluded heterogeneity state, so its model-probability diagnostics do not establish full convergence.
...
**Extrapolation to pre-publication bias.**
The package also allows us to extrapolate what the distribution of z-statistics might have looked like in the absence of publication bias.
This is achieved by either setting `extrapolate = TRUE` in the `lines()` function, or setting `plot_extrapolation = TRUE` in the `zplot()` function.

```{r, fig.cap="Social Comparison: Extrapolation Analysis"}
zplot(fit_RoBMA_Hoppen2025, plot_extrapolation = TRUE)
```

Under the inverse-weight convention, the blue line extrapolates the fitted display after removing publication-bias adjustments; it need not integrate to one.
Comparing the fitted distribution (accounting for publication bias) with the extrapolated distribution reveals the extent of the bias.
The large discrepancy between these distributions describes the model's adjustment for publication bias and its conventional inverse-weight extrapolation.
...
The observed discovery rate (ODR = 0.35) is close to the posterior mean expected discovery rate (EDR = 0.34, 95% CrI [0.24, 0.51]).
The `Missing N` diagnostic has posterior mean 57 and 95% CrI [25, 146], indicating substantial conventional extrapolation subject to the reporting assumptions above; it is not an identified count of absent studies.
...
The ChatGPT histogram contains mainly positive z-statistics.
Both fitted density curves are smooth across zero and the conventional significance thresholds.

RoBMA (blue dashed line) places more predictive mass near the positive peak than the random-effects model (black dashed line), with substantial overlap between their uncertainty bands.
The model comparison below assigns strong support to publication-bias adjustment overall; the extrapolation panel shows its effect on the implied distribution.

**Extrapolation to pre-publication bias.**
We can examine the extrapolation to assess the impact of publication bias:

```{r, fig.cap="ChatGPT: Extrapolation Analysis"}
zplot(fit_RoBMA_Wang2025, from = -2, to = 8, plot_extrapolation = TRUE)
```

The extrapolated distribution (blue line) shows the model-implied change after removing publication-bias adjustments; this comparison should be read alongside the effect estimates and diagnostics below.
...
The random-effects model estimates d = 0.829, 95% CrI [0.592, 1.070], whereas RoBMA gives a model-averaged estimate of d = 0.120, 95% CrI [-0.280, 0.556].
Both models retain substantial between-study heterogeneity, and the adjusted effect is compatible with zero.
The effect inclusion Bayes factor is 1.06, leaving evidence for its presence undecided.
The publication-bias Bayes factor is roughly 3,000, but its estimated relative Monte Carlo error is 29%, based on only 33 draws from the no-bias state.
Rare bias-state indicators have R-hat values up to 1.29, and the excluded heterogeneity state was not visited, so the detailed model probabilities remain numerically uncertain.

**Zplot summary metrics.**
The zplot summary statistics describe a different aspect of the adjustment:

```{r}
summary(as_zplot(fit_RoBMA_Wang2025))
```

The posterior mean EDR is 0.47, 95% CrI [0.37, 0.59], compared with ODR = 0.64.
The conventional `Missing N` diagnostic has posterior mean 0.02 and median zero.
Its small value does not imply weak publication-bias evidence: it describes the inverse-weight extrapolation convention, rather than the overall strength of support for bias adjustment.
...
zplot(fit_RoBMA_ManyLabs16, plot_extrapolation = TRUE)
```
...
The posterior mean effect sizes are similar (d = 0.436 for the random-effects model and d = 0.426 for RoBMA), although RoBMA has a wider credible interval.
...
The example fit's effect-inclusion indicator has R-hat = 1.35 and its effect Bayes factor has estimated relative Monte Carlo error of 126%, so that Bayes factor is not numerically precise despite the similar effect estimates.

**Zplot summary metrics.**
The posterior mean EDR is 0.58, 95% CrI [0.41, 0.68], close to ODR = 0.56.
The conventional `Missing N` diagnostic has posterior mean 0.62, median zero, and 95% CrI [0.00, 6.97]; its small value does not establish that no studies are missing.
