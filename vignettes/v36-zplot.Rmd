...
This is achieved by calling the `zplot()` function with `plot_extrapolation = TRUE`, which builds the diagnostic plot directly from a fitted model.
...
zplot(fit_RoBMA_Weingarten2018, from = -3, to = 6, by.hist = 0.25, plot_extrapolation = TRUE)
```
...
which show a moderate-to-high EDR and a small conventional `Missing N` diagnostic.
