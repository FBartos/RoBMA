...
These branches use `selection = selection_model()` by default: estimate random effects and known sampling variation are integrated, other random effects are conditioned upon, and weights are multiplied across estimates. For this specialized clustered model, within-study heterogeneity is the estimate source and the shared study effect is the other source; the usual `tau`, `rho` and `I2` summaries retain their meanings.
Setting `selection = selection_model(other_random_effects = "integrate")` instead normalizes after averaging over the shared study effect. Setting `estimate_random_effects = "condition"` retains the within-study effect during normalization and defines another target. The `known_sampling_variance` field controls the complete sampling-error source, including the independent errors specified by `sei` here; setting it to `"condition"` retains their realization during selection normalization. Explicit weightfunction priors retain their own model specification.
...
In our example, the inclusion BF for the effect is 1.54, indicating undecided evidence for an effect.
The inclusion BFs for heterogeneity and publication bias are reported as larger than 59,999, indicating extreme evidence for both components.
...
This distribution of true effect sizes can be summarized with the `summary_heterogeneity()` function. A prediction interval for a new true effect can be obtained with `pooled_effect()`:
