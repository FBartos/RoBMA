...
date: "17th of December 2025 (updated: 20th of September 2026)"
output:
...
vignette_cache_save(cached_fits["fit_simple"])
...
vignette_cache_save(cached_fits["fit"])
```
...
The multilevel approach explicitly models the clustering structure through the `cluster` argument, which:
 ...
 - adjusts for selective reporting of individual estimates, conditional on the shared study effect, in its weightfunction branches.

These branches use `selection = selection_model()` by default: estimate random effects are integrated, other random effects and known sampling context are conditioned upon, and weights are multiplied across estimates. For this specialized clustered model, within-study heterogeneity is the estimate source and the shared study effect is the other source; the usual `tau`, `rho` and `I2` summaries retain their meanings.
Setting `selection = selection_model(other_random_effects = "integrate")` instead normalizes after averaging over the shared study effect. Setting `estimate_random_effects = "condition"` retains the within-study effect during normalization and defines another target. The `known_sampling_variance` field controls declared sampling context, while residual sampling variance is always integrated; the `sei` inputs here have no sampling context. Explicit weightfunction priors retain their own model specification.
...
In our example, the inclusion BF for the effect is about 1.54, providing little evidence to distinguish the presence and absence of an average effect.
The inclusion BFs for heterogeneity and publication bias are reported as bounds above 59,999 because no corresponding null states occurred in the retained draws.
These bounds reflect limited Monte Carlo resolution; the convergence diagnostics also mark the unvisited states as not assessable.
...
In our example, we find a small effect size (*d* = 0.072, 95% CI [0.000, 0.189]), substantial heterogeneity (τ = 0.391, 95% CI [0.327, 0.460]), and nearly balanced heterogeneity allocation (ρ = 0.492, 95% CI [0.330, 0.638]).
The large heterogeneity combined with the small pooled effect suggests substantial variation in true effects.
The `summary_heterogeneity()` function summarizes the heterogeneity components, while `pooled_effect()` also reports a prediction interval for a new true effect:
...
The wide prediction interval includes both negative and positive effects: some studies may show benefits while others show harm.
