...
is_check <- ("CheckExEnv" %in% search()) ||
              ...
              !file.exists("../models/zplot/zplot_RE_Hoppen2025.RDS")
...
library(RoBMA)
dir.create("../models/zplot", recursive = TRUE, showWarnings = FALSE)
...
fit_RE_Weingarten2018 <- NoBMA(r = Weingarten2018$r_xy, n = round(Weingarten2018$N), cluster = Weingarten2018$paper_id,
...
fit_RoBMA_Weingarten2018 <- RoBMA(r = Weingarten2018$r_xy, n = round(Weingarten2018$N), cluster = Weingarten2018$paper_id,
                              sample = 10000, burnin = 10000, adapt = 10000,
                              chains = 5, parallel = TRUE, seed = 1, save = "min")
...
saveRDS(zplot_RE_Weingarten2018,    file = "../models/zplot/zplot_RE_Weingarten2018.RDS", compress = "xz")
saveRDS(zplot_RoBMA_Weingarten2018, file = "../models/zplot/zplot_RoBMA_Weingarten2018.RDS", compress = "xz")
...
zplot_RE_Hoppen2025    <- as_zplot(fit_RE_Hoppen2025)
zplot_RoBMA_Hoppen2025 <- as_zplot(fit_RoBMA_Hoppen2025)

saveRDS(zplot_RE_Hoppen2025,    file = "../models/zplot/zplot_RE_Hoppen2025.RDS", compress = "xz")
saveRDS(zplot_RoBMA_Hoppen2025, file = "../models/zplot/zplot_RoBMA_Hoppen2025.RDS", compress = "xz")
...
zplot_RE_Wang2025    <- as_zplot(fit_RE_Wang2025)
zplot_RoBMA_Wang2025 <- as_zplot(fit_RoBMA_Wang2025)

saveRDS(zplot_RE_Wang2025,    file = "../models/zplot/zplot_RE_Wang2025.RDS", compress = "xz")
saveRDS(zplot_RoBMA_Wang2025, file = "../models/zplot/zplot_RoBMA_Wang2025.RDS", compress = "xz")
...
zplot_RE_ManyLabs16    <- as_zplot(fit_RE_ManyLabs16)
zplot_RoBMA_ManyLabs16 <- as_zplot(fit_RoBMA_ManyLabs16)

saveRDS(zplot_RE_ManyLabs16,    file = "../models/zplot/zplot_RE_ManyLabs16.RDS", compress = "xz")
saveRDS(zplot_RoBMA_ManyLabs16, file = "../models/zplot/zplot_RoBMA_ManyLabs16.RDS", compress = "xz")
```
...
zplot_RE_Weingarten2018    <- readRDS(file = "../models/zplot/zplot_RE_Weingarten2018.RDS")
zplot_RoBMA_Weingarten2018 <- readRDS(file = "../models/zplot/zplot_RoBMA_Weingarten2018.RDS")
zplot_RE_Hoppen2025    <- readRDS(file = "../models/zplot/zplot_RE_Hoppen2025.RDS")
zplot_RoBMA_Hoppen2025 <- readRDS(file = "../models/zplot/zplot_RoBMA_Hoppen2025.RDS")
zplot_RE_Wang2025      <- readRDS(file = "../models/zplot/zplot_RE_Wang2025.RDS")
zplot_RoBMA_Wang2025   <- readRDS(file = "../models/zplot/zplot_RoBMA_Wang2025.RDS")
zplot_RE_ManyLabs16    <- readRDS(file = "../models/zplot/zplot_RE_ManyLabs16.RDS")
zplot_RoBMA_ManyLabs16 <- readRDS(file = "../models/zplot/zplot_RoBMA_ManyLabs16.RDS")

fit_RE_Weingarten2018    <- zplot_RE_Weingarten2018
fit_RoBMA_Weingarten2018 <- zplot_RoBMA_Weingarten2018
fit_RE_Hoppen2025    <- zplot_RE_Hoppen2025
fit_RoBMA_Hoppen2025 <- zplot_RoBMA_Hoppen2025
fit_RE_Wang2025      <- zplot_RE_Wang2025
fit_RoBMA_Wang2025   <- zplot_RoBMA_Wang2025
fit_RE_ManyLabs16    <- zplot_RE_ManyLabs16
fit_RoBMA_ManyLabs16 <- zplot_RoBMA_ManyLabs16

class(fit_RE_Weingarten2018)    <- class(fit_RE_Weingarten2018)[!class(fit_RE_Weingarten2018)       %in% "zplot_brma"]
class(fit_RoBMA_Weingarten2018) <- class(fit_RoBMA_Weingarten2018)[!class(fit_RoBMA_Weingarten2018) %in% "zplot_brma"]
class(fit_RE_Hoppen2025)    <- class(fit_RE_Hoppen2025)[!class(fit_RE_Hoppen2025)       %in% "zplot_brma"]
class(fit_RoBMA_Hoppen2025) <- class(fit_RoBMA_Hoppen2025)[!class(fit_RoBMA_Hoppen2025) %in% "zplot_brma"]
class(fit_RE_Wang2025)      <- class(fit_RE_Wang2025)[!class(fit_RE_Wang2025)           %in% "zplot_brma"]
class(fit_RoBMA_Wang2025)   <- class(fit_RoBMA_Wang2025)[!class(fit_RoBMA_Wang2025)     %in% "zplot_brma"]
class(fit_RE_ManyLabs16)    <- class(fit_RE_ManyLabs16)[!class(fit_RE_ManyLabs16)       %in% "zplot_brma"]
class(fit_RoBMA_ManyLabs16) <- class(fit_RoBMA_ManyLabs16)[!class(fit_RoBMA_ManyLabs16) %in% "zplot_brma"]
```

## Introduction

**This vignette accompanies the "Zplot: A Visual Diagnostic for Publication Bias in Meta-Analysis" manuscript [@bartos2025zcurve]. The manuscript provides the theoretical foundation and detailed methodology for zplot diagnostics, while this vignette demonstrates their practical implementation using the RoBMA R package [@RoBMA].**

Zplots provide a visual assessment of meta-analytic model fit specifically aimed at detecting misfit due to publication bias [@bartos2025zcurve].
...
The zplot approach complements statistical tests of publication bias (such as inclusion Bayes factors) with intuitive visualizations
...
2. Create zplot objects using the `as_zplot()` function.
...
The zplot works with fitted `brma`/`RoBMA` objects that retain the posterior quantities required by `as_zplot()`.
...
fit_RE_Weingarten2018 <- NoBMA(r = Weingarten2018$r_xy, n = round(Weingarten2018$N),
                               cluster = Weingarten2018$paper_id,
                               priors_effect_null = NULL, priors_heterogeneity_null = NULL,
                               sample = 10000, burnin = 10000, adapt = 10000,
...
#### Creating Zplot Objects

The `as_zplot()` function converts fitted RoBMA models into objects suitable for zplot visualization:

```{r, eval = FALSE}
# Create zplot objects
zplot_RE_Weingarten2018    <- as_zplot(fit_RE_Weingarten2018)
zplot_RoBMA_Weingarten2018 <- as_zplot(fit_RoBMA_Weingarten2018)
```
...
plot(zplot_RoBMA_Weingarten2018, from = -3, to = 6, by.hist = 0.25)
```
...
# Create histogram of observed z-statistics
hist(zplot_RoBMA_Hoppen2025)

# Add model-implied distributions
lines(zplot_RE_Hoppen2025, col = "black", lty = 2, lwd = 2)
lines(zplot_RoBMA_Hoppen2025, col = "blue", lty = 2, lwd = 2)
...
```{r}
# Extract zplot summary metrics
summary(zplot_RoBMA_Hoppen2025)
```
...
# Assess model fit
hist(zplot_RoBMA_Wang2025, from = -2, to = 8)
lines(zplot_RE_Wang2025, col = "black", lty = 2, lwd = 2, from = -2, to = 8)
lines(zplot_RoBMA_Wang2025, col = "blue", lty = 2, lwd = 2, from = -2, to = 8)
legend("topright",
       legend = c("Random-Effects", "RoBMA"),
       col = c("black", "blue"),
       lty = 2, lwd = 2)
...
plot(zplot_RoBMA_Wang2025, from = -2, to = 8)
```
...
```{r}
# Extract zplot summary metrics for ChatGPT data
summary(zplot_RoBMA_Wang2025)
```
...
```{r, eval = FALSE}
# Fit models and create zplot objects
fit_RE_ManyLabs16 <- NoBMA(d = ManyLabs16$y, se = ManyLabs16$se,
                           priors_effect_null = NULL, priors_heterogeneity_null = NULL,
                           sample = 10000, burnin = 5000, adapt = 5000,
                           chains = 5, parallel = TRUE, seed = 1)
...
hist(zplot_RoBMA_ManyLabs16)
lines(zplot_RE_ManyLabs16, col = "black", lty = 2, lwd = 2)
lines(zplot_RoBMA_ManyLabs16, col = "blue", lty = 2, lwd = 2)
legend("topleft",
       legend = c("Random-Effects", "RoBMA"),
       col = c("black", "blue"),
       lty = 2, lwd = 2)
...
```{r}
# Extract zplot summary metrics for Many Labs 2 data
summary(zplot_RoBMA_ManyLabs16)
```
...
Publication bias and QRP might produce similar patterns of results. The zplot diagnostics cannot distinguish between them; however, it might help in assessing whether the
