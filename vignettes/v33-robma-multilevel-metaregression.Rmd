...
The inclusion Bayes factors for the average effect, consumer-contact moderation, heterogeneity, and publication bias are all reported as $>59{,}999$.
Their excluded states were not visited in the retained draws, so these are reporting bounds rather than precise Bayes factor estimates; convergence diagnostics for the constant inclusion indicators remain unavailable.

```{r pooled-effect}
pooled_effect(fit_reg)
```

The model-averaged effect estimate is accompanied by considerable overall heterogeneity and a cluster-level allocation of about 0.82, indicating that most modeled heterogeneity is at the study level.
...
For jobs with no consumer contact, the marginal-means Bayes factor is approximately 0.10, providing moderate evidence for the point null.
For some or direct consumer contact, the posterior credible intervals lie above zero, but the reported KDE Bayes factors rely on kernel-tail estimates because the draws do not span zero.
Their very large numerical magnitudes should not be interpreted precisely.
...
The zplot shows the distribution of the observed $z$-statistics, computed as the effect size divided by the standard error, with dotted red vertical lines highlighting the typical steps on which publication bias operates.
...
par(mar = c(4, 4, 0, 0))
zplot(fit_reg, by.hist = 0.25, from = -4, to = 8)
```
