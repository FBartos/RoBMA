...
After fitting, the semantic catalog exposes the same quantities as `sd_total`, `var_total`, `var_prop(esid_study)`, and `var_prop(study)`.
...
After fitting, the allocation is exposed as `sd_common`, `var_common`, `var_ratio(outcome[level])`, and `sd_ratio(outcome[level])`.
...
Here `hcs(outcome | study)` is a structure-owned index model: it creates one SD per observed `outcome` level and constrains all pairs to share one public `cor` parameter.
...
Formula-random correlations use `cor` in summaries, plots, density estimation, and hypotheses; any compact backend `rho` coordinate is internal. This differs from the specialized `brma(..., cluster = ...)` model's public heterogeneity-allocation parameter `rho`.
