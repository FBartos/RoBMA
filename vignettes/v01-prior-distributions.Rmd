...
date: "28th of April 2026 (updated 22nd of June 2026)"
output:
...
print_prior(Havrankova_treatment, parameter = "facing_customer")
```
...
print_prior(Havrankova_meandif, parameter = "facing_customer")
```
...
plot_prior(Havrankova_meandif, parameter = "N")
plot_prior(Havrankova_meandif, parameter = "N", standardized_coefficients = FALSE)
```
...
### Random-effect component prior distributions

The `brma.mv()` function uses random-effect formulas to decompose residual heterogeneity into named components. Omitted or NULL `random` means a fixed-effect model, without an implicit heterogeneity component. Supply a random formula before setting its heterogeneity prior distribution or a scale formula; specialized univariate models retain their usual implicit heterogeneity.
The default prior distributions follow the same unit-information scaling as the scalar heterogeneity prior distribution, but the printed random-effect prior distribution makes the allocation logic explicit.
It shows the standard-deviation prior distributions, the allocation weights, and the implied component standard deviations.

For illustration, we use a small display-only dataset.
...
mv_dat <- data.frame(
  ...
  study   = c("s1", "s1", "s2", "s2"),
  esid    = paste0("e", 1:4),
  outcome = factor(c("c1", "c2", "c1", "c2"))
)
mv_V <- diag(rep(0.04, nrow(mv_dat)))
```

A single homogeneous random-effect component receives the default positive-normal prior distribution directly on its standard deviation:

```{r mv-random-single-default}
mv_random_single <- brma.mv(
  yi = yi, V = mv_V, measure = "GEN",
  random = ~ 1 | study,
  prior_unit_information_sd = 1,
  data = mv_dat, only_priors = TRUE
)

print_prior(mv_random_single, parameter = "random")
```

The printed output shows one block, `study`, and one standard-deviation prior distribution.
There is no allocation layer because there is only one homogeneous random-effect standard deviation.

Nested random intercepts need more than one variance component.
In that case, the default prior distribution uses one total standard deviation and a `Dirichlet(1, 1)` allocation prior distribution:

```{r mv-random-nested-default}
mv_random_nested <- brma.mv(
  yi = yi, V = mv_V, measure = "GEN",
  random = ~ 1 | study / esid,
  prior_unit_information_sd = 1,
  data = mv_dat, only_priors = TRUE
...
print_prior(mv_random_nested, parameter = "random")
```

The prior display writes the aggregate mathematically as `sigma_total`, followed by the allocation weights and implied component standard deviations.
For this nested model, the two components are the estimate-within-study component (`esid_study`) and the study component (`study`).
The allocation is on the variance scale: each component SD is `sigma_total * sqrt(w[component])`, and the component variances sum to `sigma_total^2`.
After fitting, RoBMA exposes the same quantities as `tau_total`, `tau2_total`, `tau2_prop(esid_study)`, and `tau2_prop(study)`.
Because this model has one top-level random component, RoBMA suppresses its redundant owner prefix; these are also the names used in plots, density estimation, and hypotheses.

Heterogeneous random-effect structures use the same idea inside a single block.
For example, `hcs(outcome | study)` estimates one standard deviation per outcome level and one common correlation parameter:

```{r mv-random-heterogeneous-default}
mv_random_heterogeneous <- brma.mv(
  yi = yi, V = mv_V, measure = "GEN",
  random = ~ hcs(outcome | study),
  prior_unit_information_sd = 1,
  data = mv_dat, only_priors = TRUE
)

print_prior(mv_random_heterogeneous, parameter = "random")
```

The prior display now uses `sigma_common` and shows `scale: mean_variance`.
For a heterogeneous block with $q$ standard-deviation components and allocation weight $w_j$, the $j$th component standard deviation is `sigma_common * sqrt(q * w[j])`.
Equal weights make each component SD equal to the common scale, while unequal weights move variance among the components without changing the average component variance.
After fitting, the allocation is exposed as `tau_common`, `tau2_common`, `tau2_mult(outcome[level])`, and `tau_mult(outcome[level])`.
The block also receives a uniform prior on the complete valid raw-correlation interval, `(-1 / (K - 1), 1)`, where `K` is the number of outcome levels.
This includes all negative correlations allowed by the HCS covariance matrix; it does not impose a hidden positive-correlation restriction.

Here `hcs(outcome | study)` is a structure-owned index model: it creates one SD per observed `outcome` level and constrains all pairs to share one public `rho` parameter.
It is different from an unstructured random-coefficient model such as `us(0 + outcome | study)`.
The latter obtains its factor basis from random-block contrast metadata and estimates an unrestricted correlation matrix.
Formula-random correlations use `rho` in summaries, plots, density estimation, and hypotheses. BayesTools retains its general `cor` names and compact backend coordinates internally.
See [*Multivariate and Multilevel Meta-Analysis*](v14-metafor-parity-multivariate.html#formula-semantics-and-random-effect-structures) for the full formula contract and a worked factor-level US specification.

For custom random-effect prior distributions via `BayesTools::prior_random()`, see the [*Multivariate and Multilevel Meta-Analysis*](v14-metafor-parity-multivariate.html) vignette.
