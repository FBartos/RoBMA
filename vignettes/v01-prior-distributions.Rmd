...
The output reports the total standard-deviation prior distribution, the allocation weights `w`, and the implied component standard deviations.
...
The allocation is on the variance scale, so `sigma_esid_study` and `sigma_study` are computed by multiplying `sigma_total` by the square root of the corresponding weight.
...
The printed output now shows `scale: mean_variance`.
For a heterogeneous block with $q$ standard-deviation components and allocation weight $w_j$, the $j$th component standard deviation is `sigma_total * sqrt(q * w[j])`.
Equal weights keep every component on the total-standard-deviation scale, while unequal weights move variance among the components without changing the average component variance.
The block also receives the default `Beta(1, 1)` prior distribution on the common correlation parameter `rho`.
