...
| Empirical/informed priors | Empirical priors derived from previous meta-analyses | `prior_informed_field`, `prior_informed_subfield`, `prior_informed()` |
| Custom priors | Fully user-specified priors via `prior()` and helpers | `prior_effect`, `prior_heterogeneity`, `prior_mods`, `prior_scale` |
| Publication-bias priors | Bias-adjustment models and ensembles | `model_type`, `prior_bias`, `prior_bias_null` |
...
The default prior distributions then use fractions of the UISD to specify the scale of each prior.
...
Publication-bias priors describe the bias-adjustment component (`prior_bias`) and are independent of the priors on the effect, heterogeneity, and moderators.
Because they typically attach to specialised single-model fits or to model-averaged ensembles, they are documented in the corresponding vignettes rather than here.

For *single-model* bias adjustment with `bPET()`, `bPEESE()`, or `bselmodel()`, see the [*Publication-Bias Adjustment*](11-metafor-parity-publication-bias.html) vignette: it lists the default `prior_bias` for each function, the rationale for the one-sided defaults, and the flexible alternatives that drop the truncation or the monotonicity constraint.
