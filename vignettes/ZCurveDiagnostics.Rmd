...
We illustrate the z-curve diagnostics using three empirical examples from recent meta-analyses that exhibit different degrees of publication bias. 
...
5. Use the visualization to supply statistical model comparison.
...
We demonstrate the z-curve diagnostics on three empirical meta-analyses that represent different publication bias scenarios:

1. **Social comparison as behavior change technique** [@hoppen2025meta] - An example with extreme publication bias
2. **ChatGPT effects on learning performance** [@wang2025effect] - An example with moderate publication bias  
3. **Framing effects from Many Labs 2** [@klein2018many] - An example with no publication bias (registered replication reports)
...
### Example 1: Social Comparison and Behavior Change
...
We begin by loading the social comparison dataset and examining its structure:
...
### Example 2: ChatGPT and Learning Performance
...
#### Z-Curve Analysis and Interpretation
...
### Example 3: Framing Effects from Many Labs 2
...
hist(zcurve_RoBMA_ManyLabs16, main = "Framing Effects: Model Fit Assessment")
