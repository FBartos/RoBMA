...
cached_fits <- vignette_cache(
  name     = "10-metafor-parity-multilevel",
  objects  = c("fit1_brma"),
...
This vignette illustrates how to use `RoBMA` [@RoBMA] with multilevel meta-analytic data.
It walks through the school-calendar dataset from @konstantopoulos2011fixed, showing the `brma()` call with the `cluster` argument alongside its `metafor::rma.mv()` counterpart [@metafor].
...
For the non-multilevel workflow showing more features (also applicable to the multilevel models) see the [*Bayesian Meta-Analysis*](02-bayesian-meta-analysis.html) vignette.

`RoBMA` currently supports the simple multilevel structure shown here, with one cluster grouping above the estimate level (parameterized by total heterogeneity $\tau$ and cluster-level allocation $\rho$). Broader support for multivariate models and more complex random-effects structures is planned for a future release.

## Multilevel Random-Effects Model

The school-calendar dataset from `metadat` contains 56 standardized mean differences nested in 11 school districts.
...
The posterior mean for the pooled effect tracks the REML estimate from `metafor`, with comparable cluster-level allocation `rho` and total heterogeneity `tau`.
...
The `tau` posterior concentrates around 0.3 with credible interval comparable to the profile-likelihood bounds, while the `rho` posterior favors moderate-to-high cluster-level allocation, broadly consistent with the REML point estimate of 0.67.
...
See the [*Bayesian Meta-Analysis*](02-bayesian-meta-analysis.html) vignette for the full walkthrough; the only thing that changes here is the addition of `cluster = district`.
