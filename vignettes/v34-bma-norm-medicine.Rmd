...
The results show that the inclusion Bayes factor for the effect is similar to the one reported in @bartos2021bayesian, $\text{BF}_{10} = 250$ and $\text{BF}_{\text{rf}} = 3.60$
...
The secondary y-axis (right) shows the probability of the spike (at zero) decreasing from 0.50 to approximately 0.004 (also obtainable from the model summary).
...
When comparing RoBMA to the second BMA fit, we notice a large decrease in the inclusion Bayes factor for the presence of the effect, $\text{BF}_{10} = 2.77$ vs. $\text{BF}_{10} = 250$, which still presents weak evidence for the presence of the effect.
We can further quantify the evidence in favor of publication bias with the inclusion Bayes factor for publication bias $\text{BF}_{pb} = 4.74$, which can be interpreted as moderate evidence in favor of publication bias.
