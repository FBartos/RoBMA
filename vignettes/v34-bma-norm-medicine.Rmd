...
The example fit gives an effect inclusion Bayes factor of roughly $\text{BF}_{10} = 250$ and a heterogeneity inclusion Bayes factor of $\text{BF}_{\text{rf}} = 3.60$, broadly consistent with the evidence reported in @bartos2021bayesian.
The effect Bayes factor has an estimated relative Monte Carlo error of 29%, and its inclusion indicator has R-hat = 1.15.
Its exact magnitude is therefore not numerically settled by this run.
...
The third part under the 'Conditional estimates' heading displays the conditional effect size estimate corresponding to the one reported in @bartos2021bayesian, d = 1.082, 95% CI [0.663, 1.420], and a heterogeneity estimate (not reported previously).
...
The secondary y-axis (right) shows the probability of the spike (at zero) decreasing from 0.50 to approximately 0.004 (also obtainable from the model summary), subject to the indicator-mixing limitation noted above.
...
When comparing RoBMA to the BMA fit, we notice a large decrease in the inclusion Bayes factor for the presence of the effect, from roughly 250 to $\text{BF}_{10} = 2.77$, leaving weak evidence for the presence of the effect.
We can further quantify the evidence in favor of publication bias with the inclusion Bayes factor $\text{BF}_{pb} = 4.74$, which indicates moderate evidence in favor of publication bias.

We can also compare the unadjusted and publication-bias-adjusted conditional effect size estimates.
Including models assuming publication bias into our model-averaged estimate (assuming the presence of the effect) decreases the estimated effect to d = 0.704, 95% CI [-0.327, 1.338] with a much wider credible interval.
...
par(mar = c(4, 4, 1, 4))
plot(fit_RoBMA, parameter = "mu", prior = TRUE, conditional = TRUE, xlim = c(-2, 2), ylim = c(0, 1.35), lwd = 2)
```
