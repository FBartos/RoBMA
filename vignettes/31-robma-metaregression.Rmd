...
First, we fit a frequentist meta-regression using the `metafor` R package. 
Second, we explain the Bayesian meta-regression model specification, the default prior distributions for continuous and categorical moderators, and standardized effect sizes input specification. 
...
The model is estimated on Fisher's $z$ scale and the pooled estiamtes are transformed to correlation coefficients only for reporting.
...
The current default prior distributions for Bayesian meta-analyses are calibrated to the unit information standard deviation of the effect-size measure, specified via the `measure`argument (see [*Prior Distributions*](01-prior-distributions.html).
...
We specify the Fisher's $z$ effect sizes as `yi`, their sampling variances as `vi`; and set the `measure` arguments to `"ZCOR"` which tells `BMA()` and `RoBMA()` how to interpret the input scale. 
...
As such, the results suggest that the effect size in studies using direct assessment is lower in comparison to the grand mean of the studies. The `age` regression coefficient of 0.000, 95\% CI [-0.000, 0.009] corresponds to the increase in the mean effect when increasing mean age of children by one.
...
Here, we find extreme evidence for the effect size of studies using the informant-completed questionnaires differing from zero, $\text{BF}_{10} > 10{,}000$ and strong evidence for the effect size of studies using the direct assessment differing from zero, $\text{BF}_{10} > 23.6$. 
...
RoBMA-reg reveals moderate evidence of publication bias $\text{BF}_{\text{pb}} = 4.81$. 
Furthermore, accounting for publication bias turns the previously found evidence for the overall effect into a weak evidence against the effect $\text{BF}_{10} = 1.16$ and notably reduces the mean effect estimate r = 0.066$, 95\% CI [-0.016, 0.198].
...
The estimated marginal means now suggest that studies using the informant-completed questionnaires show a much smaller effect of household chaos on child executive functions, r = 0.125, 95\% CI [0.000, 0.261] with moderate evidence in favor of the effect, $\text{BF}_{10} = 5.23$, 
while studies using direct assessment even provide weak evidence against the effect of household chaos on child executive functions, $\text{BF}_{10} = 0.561$, with most likely effect sizes around zero, r = 0.008, 95\% CI [-0.112, 0.171].
