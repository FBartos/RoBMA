...
In the original article, @kroupova2021student conducted an outstanding analysis of the data. @kroupova2021student used several publication bias adjustment techniques to assess the mean meta-analytic effect size beyond publication bias and further used Bayesian and frequentist model-averaging techniques to estimate the effect of various covariates. In this vignette, we attempt to combine both analysis using the robust Bayesian meta-analysis framework [@maier2020robust; @bartos2021no] and draw inference about the publication bias and covariates simultaneously using Bayesian model-averaging [@hinne2019conceptual; @hoeting1999bayesian; @leamer1978specification].

We start by (1) briefly outlining the data set, (2) performing a few preliminary analyses to illustrate the effect of publication bias and accounting for multiple estimates from a single study, and (3) slowly building up the robust Bayesian meta-analytic ensemble.
...
#summary(fit_wPSMA)
```
The summary function now summarizes result of a 36 models combined into a single meta-analytic ensemble. This allows us to draw inference using a diverse set of models with different assumption about the data-generating process. We find that after adjusting for publication bias and additive heterogeneity with RoBMA-PSMA, the evidence in favor of the presence of the effect essentially disappears, $\text{BF}_{10} = x \times 10^{x}$. The model-averaged effect size estimates (averaging across all specified models) further reduces to $z = -x$, 95% CI $[-x -x]$. Moreover, we find extreme evidence in favor of publication bias, $\text{BF}_{\text{pb}} = x \times 10^{x}$, and additive heterogeneity, $\text{BF}^{\text{rf}} = x \times 10^{x}$. These overall results based on RoBMA-PSMA further support the conclusions obtained by @kroupova2021student about the lack of evidence for the presence of the effect and a considerable amount of publication bias.

We can also visualize the posterior effect size estimate using the `plot()` function. We set the `effect_size = "r"` argument to transforms the effect size estimates to the correlation coefficients and add the `prior = TRUE` argument to overlay the prior distribution on top of the Figure.
```{r fit_wPSMA, dpi = 300, fig.width = 6, fig.height = 4, out.width = "75%", fig.align = "center"}
#plot(fit_wPSMA, parameter = "mu", prior = TRUE, effect_size = "r")
```
The grey and black arrows depict the prior and posterior probability of the zero effect (secondary $y$-axis). The increased length of the black arrow signifies the increased posterior probability of the absence of the effect. The continuous grey and black densities then correspond to the densities under the alternative hypotheses. The black posterior density notably shrinks toward zero which highlights that only very small effects are plausible under the alternative hypothesis.
...
#### Accounting for Covariates with Fixed-Effect Meta-Regression


#### Accounting for Covariates while Adjusting for Publication Bias

Finally, we extend the previously specified robust Bayesian meta-analyses to also estimate the effect of covariates. First, we specify a meta-regression model with a relatively wide default prior distributions for the covariates (see `?RoBMA.reg` for more details). We specify the model by extending the `formula` argument with the corresponding meta-regression specification. We further add the `test_predictors = ""` argument specifies the ensemble such all covariates are put into the model (we test for the presence of the effect of covariates in the following section using more informed prior distributions).
