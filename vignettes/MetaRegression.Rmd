---
title: "Bayesian Model-Averaged Meta-Regression"
...
csl: ../inst/apa.csl
vignette: >
  %\VignetteIndexEntry{Bayesian Model-Averaged Meta-Regression}
...
is_check <- ("CheckExEnv" %in% search()) ||
              ...
              !file.exists("../models/MetaRegression/fit.RDS")
...
fit <- readRDS(file = "../models/MetaRegression/fit.RDS")
```
...
# load data 
df <- read.csv("example-reg/HC-EF Main Analysis Data.csv")
...
# r = .16, 95% CI [.12, .20]
# Q(35) = 363.11, p < .001
rma(yi = df$Correlation, vi = df$Variance, method = "REML")
# the test for heterogeneity is a bit off but the estimate and CIs are very close

## Moderation by Executive function
# reported results:
# informant-completed questionnaires: r = .23, 95% CI [.16, .29]
# direct assessment:                   r  .11, 95% CI [.05, .16] 
rma(yi = df$Correlation, vi = df$Variance, mods = ~ as.factor(df$Approach.to.Measurement.of.Executive.Function) - 1, method = "REML")
rma(yi = df$Correlation, vi = df$Variance, mods = ~ as.factor(df$Approach.to.Measurement.of.Executive.Function), method = "REML")

## Moderation by mean Child age
# reported results:
# b = .01, 95% CI [.004, .02])
rma(yi = df$Correlation, vi = df$Variance, mods = ~ df$Child.Age..years., method = "REML")



df_reg <- na.omit(data.frame(
  r       = df$Correlation,
  se      = sqrt(df$Variance),
  measure = factor(ifelse(df$Approach.to.Measurement.of.Executive.Function == 0, "informat", "direct")),
  age     = as.numeric(scale(df$Child.Age..years.))
))

# metafor
fit.rma <- rma(yi = r, sei = se, mods = ~ measure + age, data = df_reg, method = "REML")
rma.emm <- data.frame(emmeans(emmprep(fit.rma), specs = "measure"))
rma.emm$pval <- pnorm(abs(rma.emm$emmean/rma.emm$SE), lower.tail = FALSE) * 2
rma.emm

# RoBMA
BMA    <- NoBMA.reg(~ measure + age, data = df_reg, parallel = TRUE, seed = 1, sample = 20000)
RoBMA  <- RoBMA.reg(~ measure + age, data = df_reg, parallel = TRUE, seed = 1, sample = 2000, chains = 1)

saveRDS(BMA,   file = "example-reg/BMA.RDS", compress = "xz")
saveRDS(RoBMA, file = "example-reg/RoBMA.RDS", compress = "xz")


summary(BMA, output_scale = "r")
marginal_summary(BMA)  

summary(RoBMA, output_scale = "r")
marginal_summary(RoBMA)  

marginal_plot(RoBMA, "measure", prior = TRUE, xlim = c(-1, 1), conditional = TRUE)


# create marginal posteriors figure
marg.prior.BMA           <- attr(BMA$RoBMA$inference_marginal$conditional$mu_measure$informat, "prior_samples")
marg.post.BMA.direct     <- BMA$RoBMA$inference_marginal$conditional$mu_measure$direct
marg.post.BMA.informal   <- BMA$RoBMA$inference_marginal$conditional$mu_measure$informat
marg.prior.RoBMA         <- attr(RoBMA$RoBMA$inference_marginal$conditional$mu_measure$informat, "prior_samples")
marg.post.RoBMA.direct   <- RoBMA$RoBMA$inference_marginal$conditional$mu_measure$direct
marg.post.RoBMA.informal <- RoBMA$RoBMA$inference_marginal$conditional$mu_measure$informat


pdf(file = "example-reg/marginal-posteriors.pdf", width = 9, height = 4)
{
par(mfrow = c(1, 2), mar = c(4, 4, 0, 0))

plot(NA, type = "n", xlim = c(-1, 1), ylim = c(0, 8), xlab = expression("Effect size (Cohen's "*delta*")"), ylab = "Density", bty = "n", las = 1)

lines(density(marg.prior.BMA), lty = 2, lwd = 1.5, col = "grey")  

lines(density(marg.post.BMA.direct),   lty = 1, lwd = 1.5, col = "darkblue")
lines(density(marg.post.BMA.informal), lty = 1, lwd = 1.5, col = "darkgreen")

lines(c(0, 0), c(0, 8), lty = 3)

legend("topleft",  c("Prior", "Posterior"), lty = c(2, 1), lwd = 1.5, col = c("grey", "black"), bty = "n")


plot(NA, type = "n", xlim = c(-1, 1), ylim = c(0, 8), xlab = expression("Effect size (Cohen's "*delta*")"), ylab = "Density", bty = "n", las = 1)

lines(density(marg.prior.RoBMA), lty = 2, lwd = 1.5, col = "grey")  

lines(density(marg.post.RoBMA.direct),   lty = 1, lwd = 1.5, col = "darkblue")
lines(density(marg.post.RoBMA.informal), lty = 1, lwd = 1.5, col = "darkgreen")

lines(c(0, 0), c(0, 8), lty = 3)

legend("topleft", c("Direct assesment", "Informant questionnaires"), lty = 1, lwd = 1.5, col = c("darkblue", "darkgreen"), bty = "n")
dev.off()
