...
```
We obtain similar results about the main model structure as in the down-weighted RoBMA-PSMA model not including covariates. After accounting for covariates, we find moderate evidence in the favor of the absence of the effect, $\text{BF}_{01} = 1/0.127 = 7.87$, extreme evidence in favor additive heterogeneity, $\text{BF}^{\text{rf}} = 3.49 \times 10^{76}$ (which notably decreased as a result of adding covariates), and extreme evidence in favor of publication bias, $\text{BF}_{\text{pb}} = 556.49$.

More interesting are the model-averaged meta-regression estimates -- note that all models assume presence of the predictors, therefore, the model-averaging happens across models assuming the presence/absence of the mean effect, heterogeneity, and publication bias. In general, we find rather small differences from the mean meta-analytic effect size due to covariates: the largest difference is due to the Germany location.
