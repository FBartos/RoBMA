...
The explicit `bobyqa` optimizer and larger evaluation budget make this
reference fit reproducible when all package vignettes are built in one
session; they do not change the model or the resulting estimates.
...
Because this midpoint log-rate is unbounded, there is no bounded flat default analogous to `Beta(1, 1)`, so the default `prior_lograte` is a proper normal distribution centered at the pooled crude log-rate and used independently for each `phi[i]`.
Its standard deviation defaults to `1`, matching the default IRR effect-prior scale, and can be changed with `RoBMA.options(default_lograte.sd = ...)`.
The fixed standard deviation makes the default equivariant to exposure-unit changes: rescaling every person-time shifts the prior mean by the corresponding log conversion while retaining its informativeness.
...
GLMM fits support meta-regression via `mods`, multilevel structures via `cluster`, location-scale models via `scale`, posterior summaries, plots, and predictions.
Binomial response predictions accept arm totals alone (`n1i`, `n2i`), optional event counts with those totals (`ai`, `ci`, `n1i`, `n2i`), or four-cell counts (`ai`, `bi`, `ci`, `di`); Poisson response predictions require exposures (`t1i`, `t2i`) and accept optional event counts (`x1i`, `x2i`).
Raw outcome residuals are available on the derived effect-size scale, but Pearson and internally standardized residuals are unavailable for GLMMs.
LOO-PIT diagnostics are also unavailable until a discrete PIT convention is defined.
Estimate-unit `log_lik()`, LOO, and WAIC support point or beta baserate priors and point or normal log-rate priors, including supported truncations; cluster-unit likelihood diagnostics remain unavailable.
Standalone `covratio()` and `dfbetas()` diagnostics are supported, whereas the combined `influence()` summary is not.
See the [*Bayesian Meta-Analysis*](v02-bayesian-meta-analysis.html) vignette for the general workflow and the [*Multilevel Meta-Analysis*](v10-metafor-parity-multilevel.html) vignette for multilevel structure.
