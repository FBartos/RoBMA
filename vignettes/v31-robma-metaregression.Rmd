...
We find overwhelming evidence for both with inclusion Bayes factors (`Inclusion BF`) above 14,999 (the numerical precision of Bayes factors above 100 is usually limited by the number of posterior samples).
...
Furthermore, we find moderate evidence for the null hypothesis of no moderation by mean age of the children, $\text{BF}_{\text{age}} = 0.117$ (i.e., BF for the null is 1/0.117 = 8.55).
...
Here, we find extreme evidence that the effect size for studies using the informant-completed questionnaires differs from zero, $\text{BF}_{10} > 10{,}000$, and strong evidence that the effect size for studies using direct assessment differs from zero, $\text{BF}_{10} = 20.4$.
...
In such cases, warning messages are printed and very large Bayes factors returned (like here); while the actual Bayes factor is finite, it is still too large to be computed precisely given the posterior samples.
...
Furthermore, accounting for publication bias leaves the evidence for the overall effect undecided, $\text{BF}_{10} = 1.18$, and notably reduces the mean effect estimate to r = 0.059, 95\% CI [-0.019, 0.188].
...
The estimated marginal means now suggest that studies using the informant-completed questionnaires show a much smaller effect of household chaos on child executive functions, r = 0.127, 95\% CI [0.000, 0.264] with moderate evidence in favor of the effect, $\text{BF}_{10} = 6.17$,
while studies using direct assessment even provide weak evidence against the effect of household chaos on child executive functions, $\text{BF}_{10} = 0.627$, with most likely effect sizes around zero, r = 0.010, 95\% CI [-0.112, 0.171].
