...
test_that("ordinary covariance envelopes preserve full-V density and bounded error", {
  ...
  nonmonotone <- .selection_envelope_test_call(covariance, mean, sei,
    c(1, .2, .8), z_lower = c(1, 0, -Inf), z_upper = c(Inf, 1, 0))
  best <- .selection_envelope_test_call(covariance, mean, sei,
                                        c(1, weight), vector_rule = 1L)
  expect_equal(unname(nonmonotone[["integration_diagnostics"]][1L, 1L]), 0)
