...
test_that("Test against metafor::rma.uni with mods = ~ sei ", {
  ...
  fit.marglik <- bridge_sampler(fit.bPET)
  save_fit("dat.lehmann2018-PET", fit.bPET, fit.marglik, info = list(metafor = fit_PET.metafor))
...
test_that("Test against metafor::rma.uni with mods = ~ sei (with negative effect sizes)", {
  ...
  fit.bPET    <- bPET(yi, vi, data = dat.lehmann2018, measure = "SMD", seed = 1, silent = TRUE)
  fit.marglik <- bridge_sampler(fit.bPET)
  save_fit("dat.lehmann2018-PET_neg", fit.bPET, fit.marglik, info = list(metafor = fit_PET.metafor))
