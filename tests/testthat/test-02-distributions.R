...
test_that("order-61 Hermite rule retains independently verified tail weights", {

  rule <- .gauss_hermite_nodes(61L)

  # Independently generated by statmod 1.5.1 and NumPy 2.2.6.
  expected_tail_nodes <- c(
    -14.4985339159001, -13.5986591566158,
    -12.8535642959130, -12.1884571927766
  )
  expected_tail_log_weights <- c(
    -105.983855144553, -93.6010844983172,
    -83.8849384291044, -75.6504332718152
  )
  even_powers      <- seq.int(0L, 10L, by = 2L)
  moments          <- vapply(even_powers, function(power) {
    sum(rule[["weights"]] * rule[["nodes"]]^power)
  }, numeric(1))
  expected_moments <- c(1, 1, 3, 15, 105, 945)

  expect_equal(rule[["nodes"]][1:4], expected_tail_nodes, tolerance = 1e-12)
  expect_equal(
    rule[["log_weights"]][1:4],
    expected_tail_log_weights,
    tolerance = 1e-12
  )
  expect_true(all(is.finite(rule[["log_weights"]])))
  expect_true(all(rule[["weights"]] > 0))
  expect_equal(sum(rule[["weights"]]), 1, tolerance = 1e-14)
  expect_equal(moments, expected_moments, tolerance = 1e-11)
})
