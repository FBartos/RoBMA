...
test_that("fixed mu and tau remain available to downstream methods", {
  ...
  for (name in fixed_model_names) {
    ...
    if (inherits(fit, "brma.mv")) {
      location <- .brma_parameter_select_entry(fit, "intercept", component = "mods")
      scale <- .brma_parameter_select_entry(fit, "tau", component = "random")
      location_draws <- as.matrix(BayesTools::parameter_draws(fit, location[["selection"]]))
      scale_draws <- as.matrix(BayesTools::parameter_draws(fit, scale[["selection"]]))
      expect_equal(c(ncol(location_draws), ncol(scale_draws)), c(1L, 1L), info = name)
      expect_equal(unique(as.vector(location_draws)), info[["mu"]], info = name)
      expect_equal(unique(as.vector(scale_draws)), info[["tau"]], info = name)
      displayed <- summary(fit, include_mcmc_diagnostics = FALSE)
      expect_true("mu" %in% rownames(displayed[["estimates_mods"]]) &&
        "tau" %in% rownames(displayed[["estimates_random"]]), info = name)
      expect_equal(displayed[["estimates_mods"]]["mu", "Mean"], info[["mu"]], info = name)
      expect_equal(displayed[["estimates_random"]]["tau", "Mean"], info[["tau"]], info = name)
    } else {
