...
test_that("Test against metafor::rma.glmm", {
  ...
  save_fit("bcg_glmm", fit_simple.brma, fit_simple.marglik, info = list(metafor = fit_simple.metafor))
  ...
  fit_simple.marglik <- bridge_sampler(fit_simple.brma)
  save_fit("nielweise2008_glmm", fit_simple.brma, fit_simple.marglik, info = list(metafor = fit_simple.metafor))
  expect_equal(fit_simple.metafor$beta[[1]],  fit_simple.brma$summary["mu","Mean"],  tolerance = 0.05)
