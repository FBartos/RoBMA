...
#'
#' @description Computes the absolute (tau, tau^2) and
...
summary_heterogeneity <- function(object, type = "ensemble", conditional = FALSE,
  ...
  # transform the data and posterior to scale used for fitting the model (to compute the relative heterogeneity)
  # transform the posterior to the output scale (to compute the absolute heterogeneity)
  ...
  if(model_scale != object$add_info[["output_scale"]]){
    object_relative <- .transform_posterior(object, object$add_info[["output_scale"]], model_scale)
  }else{
    object_relative <- object
  }
  if(object$add_info[["output_scale"]] != output_scale){
    object <- .transform_posterior(object, object$add_info[["output_scale"]], output_scale)
  }
  ...
  if(substr(type,1,1) == "e"){
    ...
    estimates <- BayesTools::ensemble_estimates_table(
      samples    = list(
        tau   = object$RoBMA$posteriors$tau,
        tau2  = object$RoBMA$posteriors$tau^2,
        I2    = .compute_I2(object_relative$RoBMA$posteriors$tau, v_tilde),
        H2    = .compute_H2(object_relative$RoBMA$posteriors$tau, v_tilde)
      ),
    ...
    if(is.null(object$RoBMA[["posteriors_conditional"]])){
      ...
      estimates_conditional <- BayesTools::ensemble_estimates_table(
        samples    = list(
          tau   = object$RoBMA$posteriors_conditional$tau,
          tau2  = object$RoBMA$posteriors_conditional$tau^2,
          I2    = .compute_I2(object_relative$RoBMA$posteriors_conditional$tau, v_tilde),
          H2    = .compute_H2(object_relative$RoBMA$posteriors_conditional$tau, v_tilde)
        ),
        parameters = c("tau", "tau2", "I2", "H2"),
        probs      = probs,
