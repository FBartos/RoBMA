...
BMA](articles/ReproducingBMA.md) or [Informed BMA in
medicine](articles/MedicineBMA.md).)
...
Meta-Analysis via Spike and Slab Algorithm](articles/FastRoBMA.md). The
...
the data set’s help file,
[`?dat.konstantopoulos2011`](https://wviechtb.github.io/metadat/reference/dat.konstantopoulos2011.html).
The data set consists of 56 studies estimating the effects of modified
school calendars on students’ achievement. The 56 studies were run in
...
function from `metafor` package. This model ignores the dependency
between effect size estimates. We use this simple model as our starting
point and as a comparison with the later models.
...
data, i.e., schools within districts, by using the
...
$k = 1,\ldots,K$, $\gamma_{k}$, systematically differ from the mean
...
true effects $\theta_{k,j}$ of each study $j = 1,\ldots J_{k}$
...
heterogeneity $\tau_{w}$ (as within). Finally, the observed effect sizes
$y_{k,j}$ that differ from the true effects $y_{k,j}$ due to random
errors $\text{se}_{k,j}$.

Mathematically, we can describe such a model as: $$\begin{aligned}
\gamma_{k} & {\sim \text{N}\left( \mu,\tau_{b}^{2} \right),} \\
\theta_{k,j} & {\sim \text{N}\left( \gamma_{k},\tau_{w}^{2} \right),} \\
y_{k,j} & {\sim \text{N}\left( \theta_{k,j},\text{se}_{k,j} \right).} \\
 & 
\end{aligned}$$ Where N() denotes a normal distribution with mean and
variance.
...
mean $\mu$ and covariance matrix S: $$\begin{aligned}
y_{k,.} & {\sim \text{MN}\left( \mu,\text{S} \right),} \\
\text{S} & {= \begin{bmatrix}
{\tau_{b}^{2} + \tau_{w}^{2} + \text{se}_{1}^{2}} & \tau_{w}^{2} & \ldots & \tau_{w}^{2} & \\
\tau_{w}^{2} & {\tau_{b}^{2} + \tau_{w}^{2} + \text{se}_{2}^{2}} & \ldots & \tau_{w}^{2} & \\
\ldots & \ldots & \ldots & \ldots & \\
\tau_{w}^{2} & \tau_{w}^{2} & \ldots & {\tau_{b}^{2} + \tau_{w}^{2} + \text{se}_{J_{k}}^{2}} & \\
 & & & & 
\end{bmatrix}.}
\end{aligned}$$ The random effects marginalization is helpful as it
allows us to sample fewer parameters from the posterior distribution
(which significantly simplifies marginal likelihood estimation via
bridge sampling). Furthermore, the marginalization allows us to properly
specify selection model publication bias adjustment models – the
marginalization propagates the selection process up through all the
sampling steps at once (we cannot proceed with the sequential sampling
as the selection procedure on the observed effect sizes modifies the
sampling distributions of all the preceding levels).

We can further re-parameterize the model by performing the following
substitution, $$\begin{aligned}
\tau^{2} & {= \tau_{b}^{2} + \tau_{w}^{2},} \\
\rho & {= \frac{\tau_{w}^{2}}{\tau_{b}^{2} + \tau_{w}^{2}},}
\end{aligned}$$ and specifying the covariance matrix using the
inter-study correlation $\rho$, total heterogeneity $\tau$, and the
standard errors $\text{se}_{.}$: $$\begin{aligned}
\text{S} & {= \begin{bmatrix}
{\tau^{2} + \text{se}_{1}^{2}} & {\rho\tau^{2}} & \ldots & {\rho\tau^{2}} & \\
{\rho\tau^{2}} & {\tau^{2} + \text{se}_{2}^{2}} & \ldots & {\rho\tau^{2}} & \\
\ldots & \ldots & \ldots & \ldots & \\
{\rho\tau^{2}} & {\rho\tau^{2}} & \ldots & {\tau^{2} + \text{se}_{J_{k}}^{2}} & \\
 & & & & 
\end{bmatrix}.}
\end{aligned}$$ This specification corresponds to the compound symmetry
...
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.041,0.211\rbrack$), and
heterogeneity, $\tau = 0.292$
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.233,0.364\rbrack$), estimates
closely correspond to the frequentist results (as we would expect from
parameter estimates under weakly informative priors).
...
`study_ids = dat$district` argument. This allows us to estimate the
...
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.017,0.346\rbrack$),
heterogeneity, $\tau = 0.308$
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.223,0.442\rbrack$), and
correlation, $\rho = 0.627$
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.320,0.864\rbrack$).
...
evidence for the presence of heterogeneity,
$\text{BF}_{\text{rf}} = 9.3 \times 10^{92}$. Moreover, we find that the
...
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.000,0.314\rbrack$),
heterogeneity, $\tau = 0.326$
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.231,0.472\rbrack$), and
correlation, $\rho = 0.659$
(${\text{95\% CI}\mspace{6mu}}\lbrack 0.354,0.879\rbrack$).
...
random effects is uniformly distributed on the $(0,1)$ interval (as in
the previous analyses). The second set of models assumes that there is
no clustering, i.e., the correlation of random effects $\rho = 0$, which
simplifies the structured covariance matrix to a diagonal matrix. Again,
we model average across models assuming the presence and absence of the
effect to account for the model uncertainty.

To specify this ‘special’ model ensemble with the
[`RoBMA()`](reference/RoBMA.md) function, we need to modify the previous
model call in the following ways. We removed the fixed effect models by
specifying the `priors_heterogeneity_null = NULL` argument.$^{1}$
...
vignette, see [Custom ensembles](articles/CustomEnsembles.md) vignette
for some hints.
