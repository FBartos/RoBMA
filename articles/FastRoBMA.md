...
meta-regression ([`RoBMA.reg()`](reference/RoBMA.reg.md)) in the `RoBMA`
R package using two different algorithms:
...
Meta-Regression](articles/MetaRegression.md) vignette for in depth
discussion of the example and guidance on applying robust Bayesian
model-averaged meta-regression. Here, we show that both algorithms
produce essentially identical results; however, spike-and-slab can be
much faster when many moderators (hence many models) are present. Note
that the spike-and-slab algorithm can be also applied in the case of
hierarchical (3-lvl) models (see [Hierarchical Bayesian Model-Averaged
Meta-Analysis](articles/HierarchicalBMA.md)) which makes hierarchical
approximate selection models feasible.
...
[`marginal_summary()`](reference/marginal_summary.md) function to
