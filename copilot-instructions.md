...
- **Fast testing**: When developing, run
  `devtools::test(filter = "topic")` to focus on specific areas, model
  fits are cached to speed up tests (`devtools::test(filter = "fit")`
  must be run first to generate cached fits)
...
- **Base class**: `RoBMA` (S3 object containing `models` list, `data`,
  `fit_control`, etc.)
- **Subclasses**: `RoBMA.reg` (meta-regression), `BiBMA`
  (binomial-normal for binary outcomes)
- **Class hierarchy**: `c("BiBMA", "RoBMA")` or
  `c("RoBMA", "RoBMA.reg")` for method dispatch
- **Key methods**: [`summary.RoBMA()`](reference/summary.RoBMA.md),
  [`plot.RoBMA()`](reference/plot.RoBMA.md),
...
- `R/priors.R`: Re-exports from `BayesTools` (e.g.,
  ...
  [`prior_PET()`](reference/prior_PET.md))
...
- `R/inference-and-model-averaging.R`: BMA weights, Bayes factors,
  posterior mixing
- `R/check-input-and-settings.R`: Input validation and
  [`check_setup()`](reference/check_setup.md) for previewing ensembles
...
- **Preview ensemble**: Use [`check_setup()`](reference/check_setup.md)
  to see model structure before fitting
...
- `MetaRegression.Rmd`: [`RoBMA.reg()`](reference/RoBMA.reg.md) with
  moderators
